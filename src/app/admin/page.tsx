    ...
    <div style={{ fontFamily: "'Pretendard Variable', Pretendard, -apple-system, sans-serif", minHeight: '100vh', background: '#F7F6F3' }}>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px 60px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <span style={{ background: '#7C3AED', color: '#fff', fontSize: 11, fontWeight: 800, padding: '5px 12px', borderRadius: 8 }}>ADMIN</span>
        </div>

        {/* 탭 */}
        <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '1px solid #F0EDE8' }}>
        ...
        {tab === 'settlements' && (
          <div>
            ...
            {SELLERS.slice(0, 3).map(s => (
              <div key={s.id} style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', border: '1px solid #F0EDE8', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <button style={{ background: '#111827', color: '#fff', border: 'none', borderRadius: 10, padding: '9px 16px', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>입금 처리</button>
              </div>
            ))}
          </div>
        )}
