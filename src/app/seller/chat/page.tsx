      ...
      <main style={{ maxWidth: 680, margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        {loading ? (
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: '60px 20px', border: '1px solid #F0EDE8', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: '#6B7280', marginBottom: 6 }}>아직 채팅이 없어요</div>
            <div style={{ fontSize: 13, color: '#9CA3AF' }}>구매자가 채팅을 시작하면 여기에 표시됩니다</div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {rooms.map(room => (
              <Link key={room.id} href={`/seller/chat/${room.id}`} style={{ textDecoration: 'none' }}>
                <div style={{ background: '#fff', borderRadius: 14, padding: '16px 18px', border: `1px solid ${room.unreadCount > 0 ? '#BFDBFE' : '#F0EDE8'}`, display: 'flex', alignItems: 'center', gap: 14, transition: 'box-shadow 0.15s', cursor: 'pointer' }}
                  ...
                  <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg,#10B981,#059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 900, color: '#fff', flexShrink: 0, overflow: 'hidden' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 13, color: '#6B7280', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {room.messages[0] ? room.messages[0].content : '채팅을 시작해보세요'}
                    </div>
                  </div>
                  {room.unreadCount > 0 && (
