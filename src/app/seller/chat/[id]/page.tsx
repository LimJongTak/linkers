  ...
  return (
    <div style={{ fontFamily: "'Pretendard Variable', sans-serif", minHeight: '100vh', background: '#F7F6F3', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #F0EDE8', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 60, zIndex: 50 }}>
        <Link href="/seller/chat" style={{ color: '#6B7280', textDecoration: 'none', fontSize: 20, lineHeight: 1 }}>←</Link>
        <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#10B981,#059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 900, color: '#fff', overflow: 'hidden', flexShrink: 0 }}>
          {room?.user?.profile_image
            ? <img src={room.user.profile_image} alt={buyerName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} referrerPolicy="no-referrer" />
            : buyerName[0]
          }
        </div>
        <div>
          <div style={{ fontSize: 15, fontWeight: 800, color: '#111827' }}>{buyerName}</div>
          <div style={{ fontSize: 11, color: '#9CA3AF' }}>구매자</div>
        </div>
      </div>
            ...
            return (
              <div key={msg.id}>
                ...
                <div style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: 6 }}>
                  {!isMine && (
                    <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#10B981,#059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 900, color: '#fff', flexShrink: 0, overflow: 'hidden' }}>
                      {room?.user?.profile_image
                        ? <img src={room.user.profile_image} alt={buyerName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} referrerPolicy="no-referrer" />
                        : buyerName[0]
                      }
                  ...
                  <div style={{ maxWidth: '70%' }}>
                    {!isMine && (
                      <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 3, marginLeft: 2 }}>{buyerName}</div>
                    )}
                    <div style={{
                      background: isMine ? '#0369A1' : '#fff',
      ...
      <div style={{ background: '#fff', borderTop: '1px solid #F0EDE8', padding: '12px 16px', position: 'sticky', bottom: 0 }}>
        <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', gap: 10, alignItems: 'flex-end' }}>
          ...
          <button onClick={sendMessage} disabled={!input.trim() || sending}
            style={{ background: !input.trim() || sending ? '#E5E7EB' : '#0369A1', color: !input.trim() || sending ? '#9CA3AF' : '#fff', border: 'none', borderRadius: 12, width: 44, height: 44, fontSize: 18, cursor: !input.trim() || sending ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background 0.15s' }}>
            {sending ? '...' : '↑'}
          </button>
        </div>
      </div>
