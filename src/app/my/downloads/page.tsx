  ...
  return (
    <div style={{ fontFamily: "'Pretendard Variable', Pretendard, -apple-system, sans-serif", minHeight: '100vh', background: '#F7F6F3' }}>
      ...
      <main style={{ maxWidth: 760, margin: '0 auto', padding: '32px 24px 60px' }}>
        ...
        {DEMO_FILES.map(f => (
          <div key={f.id} style={{ background: '#fff', borderRadius: 18, padding: '18px 20px', border: '1px solid #F0EDE8', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{TYPE_ICON[f.type] ?? '📁'}</div>
