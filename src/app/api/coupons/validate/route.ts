...
export async function POST(req: NextRequest) {
  try {
    ...
    const program = await db.program.findUnique({
      where: { id: programId },
      select: { id: true, price: true, seller_id: true, title: true },
    })
    ...
    if (coupon.type === 'percentage') {
      discount = Math.floor(program.price * coupon.discount_value / 100)
      if (coupon.max_discount) discount = Math.min(discount, coupon.max_discount)
    } else {
      discount = Math.min(coupon.discount_value, program.price)
    }
